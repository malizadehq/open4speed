...
modelo4s::modelo4s(const char* filename) {
    ...
    for (int i = 0; i < textureCount; i++) {
        ...
        m->tuv = new GLfloat[m->triangleCount[cutX * cutY] * 3 * 2];
        for (int j = 0; j < m->triangleCount[cutX * cutY]; j++) {
            m->tid[j * 6 + 0] = 1.0f / (float)((j + offset) % 256);
            m->tid[j * 6 + 1] = 1.0f / (float)((j + offset) / 256);
            m->tid[j * 6 + 2] = 1.0f / (float)((j + offset) % 256);
            m->tid[j * 6 + 3] = 1.0f / (float)((j + offset) / 256);
            m->tid[j * 6 + 4] = 1.0f / (float)((j + offset) % 256);
            m->tid[j * 6 + 5] = 1.0f / (float)((j + offset) / 256);
            m->tuv[j * 6 + 0] = 0;
            m->tuv[j * 6 + 1] = 0;
            m->tuv[j * 6 + 2] = 1;
            m->tuv[j * 6 + 3] = 0;
        ...
        offset += m->triangleCount[cutX * cutY];
        m->colora = colora;
        m->colord = colord;
        m->colors = colors;
        for (int j = 0; j < m->triangleCount[cutX * cutY]; j++) {
        ...
        glBufferData(GL_ARRAY_BUFFER, size * 12, NULL, GL_STATIC_DRAW);
        ...
        glBufferSubData(GL_ARRAY_BUFFER, size * 10, size * 2, m->tuv);
