...
glsl::glsl(std::vector<char*> vert, std::vector<char*> frag) {
    ...
    attribute_v_tuv = glGetAttribLocation(id, "v_tuv");
}
...
void glsl::bind() {
    ...
    if (attribute_v_tuv != -1)
        glEnableVertexAttribArray(attribute_v_tuv);
}
...
void glsl::attrib(int size) {
    ...
    if (attribute_v_tuv != -1)
        glVertexAttribPointer(attribute_v_tuv, 2, GL_FLOAT, GL_FALSE, 0, ( const GLvoid *) (size * 10));
}
