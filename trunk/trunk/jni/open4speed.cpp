...
void display(void) {
    ...
    if (mode == 1) {
        xrenderer->setRTT(2, 0);

        /// set camera
        xrenderer->perspective(90, aspect, 0.1, viewDistance);
        float cameraX = 100;
        float cameraY = 100;
        float cameraZ = 100;
        xrenderer->lookAt(cameraX,cameraY,cameraZ,cameraX-1,cameraY-1,cameraZ-1,0,1,0);

        /// render track
        xrenderer->enable[1] = false;
        xrenderer->overshader = getShader("light_id");
        xrenderer->renderModel(trackdata);
        xrenderer->overshader = 0;

        // render RTT
        xrenderer->setRTT(2, 1);
    }

    if (mode == 2) {
        xrenderer->setRTT(3, 0);
        xrenderer->overshader = getShader("lightmap");
        xrenderer->renderRTT(2);
        xrenderer->overshader = 0;
        xrenderer->setRTT(3, 1);
    }
